...
<main>
    <!-- introduction -->
     <section class="bg-white dark:bg-gray-900">
        <div class="container mx-auto px-6 py-10 text-center">
            <h2 class="text-3xl font-semibold capitalize dark:text-white lg:text-4xl">Sản phẩm Đồ Dùng 1 Lần cho Ngành F&B</h2>
            <div class="mx-auto mt-6 flex justify-center">
                <span class="inline-block h-1 w-20 rounded-full bg-blue-500"></span>
                <span class="mx-1 inline-block h-1 w-20 rounded-full bg-blue-500"></span>
                </div>
            <p class="py-2">BLPCUP cung cấp nhiều loại sản phẩm từ ly nhựa, ly giấy đến ly thủy tinh, tô giấy, nắp ly, và các phụ kiện đi kèm như ống hút và thìa, phục vụ cho ngành F&B. Các sản phẩm đều được chọn lọc kỹ càng, đảm bảo chất lượng cao và an toàn cho người sử dụng. Chúng tôi cam kết mang đến những sản phẩm tiện dụng và thẩm mỹ cho mọi nhu cầu kinh doanh của bạn.</p>
            <p class="font-semibold">TƯ VẤN 24/7 TẬN TÂM THIẾT KẾ MIỄN PHÍ – TIÊU CHUẨN HÀNG XUẤT CAM KẾT HÀNG KHÔNG BONG TRÓC, KHÔNG RẠN NỨT BỀN MÀU THEO THỜI GIAN</p>
        </div>
     </section>
     <!-- category -->
     <section class="container mx-auto p-10 md:py-12 px-0 md:p-8 md:px-0">
        <section
            class="p-5 md:p-0 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 items-start ">
            @foreach ($dataCategories as $category)
            <a href="{{ route('productList', [$category->category_slug,$category->category_code]) }}">
            <section class="p-5 text-center transform duration-500 hover:-translate-y-2 cursor-pointer">
    ...
    <section class="bg-white dark:bg-gray-900">
        ...
        <div
                ...
                @foreach ($dataBanners as $key => $banner)
            <button
                type="button"
                data-te-target="#carouselExampleIndicators"
                data-te-slide-to="{{ $key }}"
                @if ($key === 0) data-te-carousel-active aria-current="true" @endif
            ...
            @endforeach   
                ...
                @foreach ($dataBanners as $key => $banner)
                    <div
                        class="relative float-left -mr-[100%] {{ $key === 0 ? '' : 'hidden' }} w-full transition-transform duration-[600ms] ease-in-out motion-reduce:transition-none"
                        data-te-carousel-item
                        @if ($key === 0) data-te-carousel-active @endif>
                        <img
    ...
    <div class="container mx-auto px-6 py-10">
        <div class="p-1">
                    <h2 class="text-center text-3xl font-semibold capitalize text-red-500 dark:text-white lg:text-4xl">SẢN PHẨM BÁN CHẠY</h2>
                    <div class="mx-auto mt-6 flex justify-center">
        <span class="inline-block h-1 w-20 rounded-full bg-blue-500"></span>
        <span class="mx-1 inline-block h-1 w-20 rounded-full bg-blue-500"></span>
        </div>
...
<!-- Create By Joker Banny -->
        <div class="flex justify-center items-center py-5 mt-2">
        <div class="bg-white dark:bg-gray-800 dark:border-gray-700 md:px-1 md:grid md:grid-cols-3 lg:grid-cols-4 gap-5 md:space-y-0">
            @foreach ( $dataProduct as $product)
                <div class="max-w-sm  px-6 pt-6 pb-2 border border-gray-300 rounded-lg shadow-sm transform hover:scale-105 transition duration-500">
                    <div class="relative">
                        <img class="w-full rounded-xl" src="{{$product->product_img}}g" alt="Colors" />
                        <p class="absolute top-0 bg-yellow-300 text-gray-800 font-semibold py-1 px-3 rounded-br-lg rounded-tl-lg">FREE</p>
                    </div>
                    <h1 class="mt-4 text-gray-800 text-lg font-bold cursor-pointer">{{ $product->product_name }}</h1>
                    <div class="my-4">
                        <button class="mt-4 text-xl w-full text-white bg-indigo-600 py-2 rounded-xl shadow-lg">Xem chi tiết</button>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="h-[32rem] bg-gray-100 dark:bg-gray-800">
        <div class="container mx-auto px-6 py-10">
        <h1 class="text-center text-3xl font-semibold capitalize text-gray-800 dark:text-white lg:text-4xl">TỰ nghĩ hoặc bỏ nội dung</h1>
        ...
        <p class="mx-auto mt-6 max-w-2xl text-center text-gray-500 dark:text-gray-300">bốc phét vào</p>
        </div>
    </div>
    ...
    <div class="container mx-auto -mt-72 px-6 py-10 sm:-mt-80 md:-mt-96">
        ...
        <div class="flex flex-col items-center rounded-xl border p-4 dark:border-gray-700 sm:p-6">
            <img class="aspect-square w-full rounded-xl object-cover" src="https://bizweb.dktcdn.net/100/432/504/themes/930560/assets/col_banner_img_1.jpg?1749442573130" alt="" />
    ...
    <section class="bg-white dark:bg-gray-900">
    <div class="container mx-auto px-6 py-10">
        <h1 class="text-center text-3xl font-semibold capitalize text-green-800 dark:text-white lg:text-2xl">NHỮNG ĐIỀU CHÚ Ý KHI CHỌN SẢN PHẨM CHO QUÁN</h1>
        <div class="mx-auto mt-6 flex justify-center">
        <span class="inline-block h-1 w-40 rounded-full bg-blue-500"></span>
        <span class="mx-1 inline-block h-1 w-3 rounded-full bg-blue-500"></span>
        <span class="inline-block h-1 w-1 rounded-full bg-blue-500"></span>
        ...
        @foreach ($dataNote as $note)
            <div>
                <p class="flex items-center text-center text-gray-500 lg:mx-8">{{ $note->note_content }}</p>
    ...
    <!-- article -->
    <section class="bg-lime-100 dark:bg-gray-900">
    <div class="container mx-auto px-6 py-10">
        <div class="text-center">
        <h1 class="text-3xl font-semibold capitalize text-lime-700 dark:text-white lg:text-4xl">BÀI VIẾT LIÊN QUAN</h1>
        <div class="mx-auto mt-6 flex justify-center">
        <span class="inline-block h-1 w-20 rounded-full bg-blue-500"></span>
        <span class="mx-1 inline-block h-1 w-20 rounded-full bg-blue-500"></span>
        </div>
    </div>
        <div class="mt-8 grid grid-cols-1 gap-8 md:mt-16 md:grid-cols-3 xl:grid-cols-4">
        @foreach ($dataArticle as $article)
        <div>
            <div class="relative">
            <img class="h-64 w-full rounded-lg object-cover object-center lg:h-80" src="{{ $article->article_img }}" alt="{{ $article->article_name }}" />
            ...
            <h1 class="mt-6 text-xl font-semibold text-gray-800 dark:text-white">{{$article->article_title}}</h1>
